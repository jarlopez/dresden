...
public class CRBChecks extends TestBase {
    ...
    public static void checkCausalDelivery(int numNodes, int numChurnNodes) {
        for (int i = 1; i < numNodes; i++) {
            ...
            boolean senderIncorrect = SimUtil.isChurnNode(i, numNodes, numChurnNodes);
            if (causations == null) {
                assertTrue(senderIncorrect);
            } else {
                for (String pair : causations) {
                    ...
                    for (int j = 1; j < numNodes; j++) {
                        ...
                        boolean receiverIncorrect = SimUtil.isChurnNode(j, numNodes, numChurnNodes);
                        if (delivers == null) {
